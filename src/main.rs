...
struct Args {
    ...
    #[clap(short = 'w', long, value_parser, default_value_t = 0)]  // Using 'w' like wait
    delay: u64,
}
...
fn parse_headers(header_args: &[String]) -> HashMap<String, String> {
...
async fn perform_attack(
    ...
    // Print most common response codes if we had successful requests
    // This would require tracking response codes, which we're not doing currently
    // But this is where we would add more detailed statistics
}